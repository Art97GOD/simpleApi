...
def get_statistics():
    ...
    cursor.execute(
        ...
        SELECT 
            ...
            SUM(CASE WHEN b.status = 'completed' THEN b.price ELSE 0 END) as period_revenue
        FROM users u
        LEFT JOIN bookings b ON u.id = b.client_id OR u.id = b.master_id
    ...
    return jsonify({
        ...
        "overall": {
            ...
            "bookings": {
                ...
                "completion_rate": round((stats[5] or 0) / (stats[3] or 1) * 100, 2) if stats[3] else 0
            },
            "revenue": float(stats[4] or 0)
        },
        "current_period": {
            "new_users": period_stats[0] or 0,
            "new_masters": period_stats[1] or 0,
            "new_bookings": period_stats[2] or 0,
            "revenue": float(period_stats[3] or 0)
        }
    })
...
def get_sales_report():
    ...
    cursor.execute(
        ...
        SELECT 
            ...
            SUM(b.price) as daily_revenue,
            AVG(b.price) as avg_ticket
        FROM bookings b
...
def get_best_masters():
    ...
    cursor.execute(
        """
        SELECT 
            u.id, CONCAT(u.first_name, ' ', u.last_name) as master_name,
            m.rating, 
            (SELECT COUNT(*) FROM reviews WHERE master_id = m.id) as review_count,
            (SELECT COUNT(*) FROM bookings WHERE master_id = m.id AND status = 'completed') as completed_orders,
            COUNT(DISTINCT b.id) as recent_bookings,
            SUM(CASE WHEN b.status = 'completed' THEN b.price ELSE 0 END) as recent_revenue,
            m.verified
        FROM users u
        ...
        WHERE u.role = 'master'
        GROUP BY u.id, u.first_name, u.last_name, m.rating, m.verified, m.id
        ORDER BY m.rating DESC, completed_orders DESC
        LIMIT %s
        """,
        (limit,)
    )
...
def get_booking_metrics():
    ...
    return jsonify({
        "metrics": {
            ...
            "time_metrics": {
                "avg_hours_to_complete": float(time_metrics[0]) if time_metrics[0] else 0,
                "avg_booking_value": float(time_metrics[1]) if time_metrics[1] else 0
            }
        }
    })
...
def get_all_bookings():
    ...
    query = """
        SELECT b.id, b.status, b.price,
               b.created_at, b.scheduled_time, b.completed_at,
...
def get_all_users():
    ...
    query = """
        SELECT u.id, u.email, u.first_name, u.last_name, u.role, u.phone, 
               ...
               m.verified, m.rating,
               (SELECT COUNT(*) FROM bookings WHERE client_id = u.id) as total_bookings
        FROM users u
        ...
        "users": [
            {
                ...
                "master_info": {
                    "verified": u[8],
                    "rating": float(u[9]) if u[9] else 0
                } if u[8] is not None else None,
                "total_bookings": u[10] or 0
            } for u in users
        ],
        "pagination": {
            "page": page,
...
def toggle_user_verification(user_id):
    ...
    if not master:
        return jsonify({"error": "user is not a master"}), 400
    ...
    cursor.execute(
        ...
        SET verified = NOT verified,
            verified_at = CASE WHEN NOT verified THEN NOW() ELSE NULL END
        WHERE user_id = %s
